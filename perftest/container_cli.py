...
def main():
    ...
    baseline_parser = subparsers.add_parser(
        ...
        help='Generate a baseline profile using Device Farm'
    )
    baseline_parser.add_argument('--branch', required=True, help='Git branch name')
    baseline_parser.add_argument('--commit', required=True, help='Git commit hash')
    baseline_parser.add_argument('--project-arn', required=True, help='AWS Device Farm project ARN')
    baseline_parser.add_argument('--device-pool-arn', required=True, help='AWS Device Farm device pool ARN')
    baseline_parser.add_argument('--run-name', help='Optional test run name')

    # full-baseline-pipeline command
    full_baseline_parser = subparsers.add_parser(
        'full-baseline-pipeline',
        help='Run complete baseline pipeline: build → generate'
    )
    full_baseline_parser.add_argument('--branch', required=True, help='Git branch name')
    ...
    subparsers.add_parser(
        ...
        help='Interactive baseline profile generation workflow'
    )
    subparsers.add_parser(
        'full-baseline-pipeline-interactive',
        help='Interactive full baseline pipeline workflow (build → generate)'
    )
    ...
    elif args.command == 'generate-baseline-profile':
        non_interactive_generate_baseline_profile(
            branch=args.branch,
            commit=args.commit,
            project_arn=args.project_arn,
    ...
    elif args.command == 'full-baseline-pipeline':
        non_interactive_full_baseline_pipeline(
            branch=args.branch,
            commit=args.commit,
            project_arn=args.project_arn,
    ...
    elif args.command == 'generate-baseline-profile-interactive':
        from perftest.interactive import generate_baseline_profile_interactive
        generate_baseline_profile_interactive()
    elif args.command == 'full-baseline-pipeline-interactive':
        from perftest.interactive import full_baseline_pipeline_interactive
        full_baseline_pipeline_interactive()
    else:
        # No command provided
